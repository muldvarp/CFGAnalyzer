...
type constraintMaker = int -> int -> Coding.constraints
...
let extractWord alphabet j =
  List.map 
    (fun h -> try
                List.find (fun t -> 1 = getValue (T(t,h))) 
                          alphabet
...
let emptiness cfgs = 
  ...
  let retrieve lower_k upper_k =
    let j = List.find (fun h -> message 3 (fun _ -> "  Checking whether variable " ^ showVar (N(cfg,cfg.start,0,h)) ^ " was set to 1.\n");
                                1 = getValue (N(cfg,cfg.start,0,h))) 
                      (range lower_k upper_k) 
    in
    extractWord cfg.alphabet j
  in 
  {
  identifier = "emptiness";
